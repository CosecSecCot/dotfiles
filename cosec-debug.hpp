...
// just sudo copy it into your include directory
// generally present at: /usr/include/c++/11/
// (version can vary)
// DON'T GIVE SUDO PERMISSIONS TO YOUR EDITOR !!!
...
inline void _print(char t) { cerr << '\'' << t << '\''; }
inline void _print(string t) { cerr << '"' << t << '"'; }
inline void _print(const char *t) { cerr << '"' << t << '"'; }
inline void _print(bool t) { cerr << (t ? "true" : "false"); }
inline void _print(nullptr_t t) { cerr << "nullptr"; }

template <typename T>
void _print_recursive(const T &container, int depth = 0,
                      bool force_break = false) {
  if constexpr (is_iterable_v<T> && !std::is_same_v<T, std::string>) {
    cerr << "[";
    bool first = true;
    bool all_non_iterable = std::all_of(
        std::begin(container), std::end(container), [](const auto &elem) {
          return !is_iterable_v<decltype(elem)> &&
                 !std::is_array_v<decltype(elem)>;
        });
...
void _print_c_array(const T (&arr)[N], int depth = 0) {
  cerr << "[";
...
// For vector
template <class T> inline void _print(vector<T> v) { _print_recursive(v); }
